...
$order_id_err = $customer_name_err = $status_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($order_id_err) && empty($customer_name_err) && empty($status_err)) {
        ...
        $sql = "INSERT INTO orders (order_id, customer_id, status, order_date) VALUES (?, ?, ?, CURDATE())";
        
        if ($stmt = $conn->prepare($sql)) {
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("sis", $param_order_id, $param_customer_id, $param_status);
...
<body>
    <div class="container">
        <h2>Process Order</h2>
        <p>Please fill this form to process an order.</p>
